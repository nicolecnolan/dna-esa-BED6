    ...
    for (store, _, _) in initialize_pinecone(checkpoint_queue, data_queue, args.device):
        ...
        for read_length, insertion_rate, deletion_rate, quality, \
            ...
            for sample in tqdm(mapped_reads):
                ...
                dictionary_of_values[sample.read.query_sequence] = [sample.reference]
            ...
            returned_dictionary = results[1]
            
            import pickle
            with open('test_cache/logs/read_match.pickle', 'wb') as handle:
                pickle.dump(returned_dictionary, handle, protocol=pickle.HIGHEST_PROTOCOL)
